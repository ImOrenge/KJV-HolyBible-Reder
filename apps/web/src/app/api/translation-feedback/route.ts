...
export async function POST(request: Request) {
  ...
  const supabase = bearerAccessToken ? createBearerClient(bearerAccessToken) : await createClient();
  const {
    data: { user },
    error: userError,
  } = bearerAccessToken ? await supabase.auth.getUser(bearerAccessToken) : await supabase.auth.getUser();
