...
export async function PATCH(request: Request, context: RouteContext) {
  ...
  if (!payload) return personalNoteJson({ error: "저장할 노트가 없습니다." }, { status: 400 });
  try {
    ...
    if (saveError) {
      if (saveError.message.includes("note_revision_conflict")) {
        ...
        return personalNoteJson({ error: "다른 기기에서 이 노트가 수정되었습니다.", code: "note_revision_conflict", current: current ? mapPersonalNoteRow(current, auth.user.id) : null }, { status: 409 });
      }
    ...
    return personalNoteJson({ note: mapPersonalNoteRow(note, auth.user.id), saved });
  } catch (error) {
    return personalNoteJson({ error: error instanceof Error ? error.message : "노트를 저장하지 못했습니다." }, { status: 400 });
  }
}
...
export async function DELETE(request: Request, context: RouteContext) {
  ...
  if (error) return personalNoteJson({ error: error.message }, { status: 500 });
  return personalNoteJson({ ok: true, archived: !permanent });
}
