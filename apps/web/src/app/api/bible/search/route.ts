...
import { getBook, searchFixtureVerses } from "@/lib/bible-repository";
import {
  clampSearchLimit,
  clampSearchOffset,
...
const localFixtureSource: BibleSource = { name: "Local fixture", module: "KJV fixture", version: "dev" };
...
export async function GET(request: Request) {
  try {
    ...
    if (query.length > 80) {
      return jsonWithCors({ error: "Search query is too long." }, { status: 400 });
    }
    ...
    return jsonWithCors({ ...response, lang, sort });
  } catch (error) {
    if (canUseLocalBibleFallback()) {
      const url = new URL(request.url);
      const query = normalizeBibleSearchQuery(url.searchParams.get("q") ?? "");
      const lang = normalizeBibleSearchLanguage(url.searchParams.get("lang"));
      const sort = normalizeBibleSearchSort(url.searchParams.get("sort"));
      const testament = normalizeTestamentFilter(url.searchParams.get("testament"));
      const bookId = normalizeBookFilter(url.searchParams.get("bookId"));
      const limit = clampSearchLimit(url.searchParams.get("limit"));
      const offset = clampSearchOffset(url.searchParams.get("offset"));
      const verses = searchFixtureVerses(query)
        .filter((verse) => !bookId || verse.bookId === bookId)
        .filter((verse) => {
          const book = getBook(verse.bookId);
          return !testament || (testament === "OT" ? book?.testament === "old" : book?.testament === "new");
        });
      const response: BibleSearchResponse = {
        query,
        normalizedQuery: lang === "ko" ? normalizeKoreanSearchText(query) : query,
        lang,
        sort,
        total: verses.length,
        source: localFixtureSource,
        verses: verses.slice(offset, offset + limit),
      };
      return jsonWithCors(response);
    }
