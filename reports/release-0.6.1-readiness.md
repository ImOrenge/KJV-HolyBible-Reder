...
`main@c04431ec`을 기준으로 첫 로그인 온보딩과 QT 커뮤니티·랭킹 기능을 UI/UX 개편 브랜치에 통합했다. 웹·Expo 빌드, 원격 커뮤니티 API, 마이그레이션, DB lint, 반응형 UI와 EAS production AAB 검증을 통과했다.

## 릴리즈 범위

- 첫 로그인 프로필 온보딩: 닉네임, 이름, 호칭, 프로필 사진.
- 웹·Expo Supabase Google OAuth와 이메일 인증.
- 독립 QT 커뮤니티: 웹 sidebar route, 모바일 오늘 영역 진입 화면, QT 나눔과 관련 구절 토론.
- 커뮤니티 내부 탭: 피드, 내 참여, 랭킹, 설정.
...
- 최신 온보딩·Reader V2 구조와 커뮤니티 route의 import, view type, shared export 충돌을 통합했다.
- Supabase 공개 설정이 없는 개발 환경에서 로그인·회원가입 page가 runtime error 대신 명시적 비활성 상태를 표시하게 했다.
...
- UI/UX 개편 브랜치의 원격 push, `main` 재병합, 새 태그와 배포는 이번 작업에서 수행하지 않는다.
